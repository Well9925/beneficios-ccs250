...
async function carregarBeneficios() {
  ...
  try {
    ...
    try {
      ...
      console.log(
        `Erro ao interpretar JSON. URL: ${url}, status: ${response.status}, mensagem: ${err.message}`
      );
      console.log(`String recebida: ${text}`);
    ...
    console.log(
      `Erro ao buscar benefícios. URL: ${url}, status: ${err.status ?? 'desconhecido'}, mensagem: ${err.message}`
    );
    mostrarErro('Não foi possível carregar os benefícios.');
  }
}
